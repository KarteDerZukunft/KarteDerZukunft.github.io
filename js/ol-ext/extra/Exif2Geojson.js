...
(function () {
  ...
  function getDMS(l) {
    if (l)
      return getNumber(l[0]) + getNumber(l[1]) / 60 + getNumber(l[2]) / 3600;
    else return null;
  }
  // Constructor
      ...
      EXIF.getData(f, function () {
        ...
        if (this.exifdata.GPSLongitudeRef) {
          ...
          fjs.geometry.coordinates = [
            (this.exifdata.GPSLongitudeRef == "E" ? 1 : -1) *
              getDMS(this.exifdata.GPSLongitude),
            (this.exifdata.GPSLatitudeRef == "N" ? 1 : -1) *
              getDMS(this.exifdata.GPSLatitude),
          ];
          if (this.exifdata.GPSAltitude)
            fjs.geometry.coordinates.push(getNumber(this.exifdata.GPSAltitude));
          fjs.properties.url = this.src || this.name;
          if (this.exifdata.ImageDescription)
            fjs.properties.description = this.exifdata.ImageDescription;
          if (options.date && this.exifdata.DateTime)
            fjs.properties.date = this.exifdata.DateTime;
          // Camera info
          if (options.camera) {
            if (this.exifdata.Make) fjs.properties.make = this.exifdata.Make;
            if (this.exifdata.Model)
              fjs.properties.model = this.exifdata.Model.replace(
          ...
          if (options.image) {
            ...
            if (this.exifdata.ImageHeight)
              fjs.properties.height = this.exifdata.ImageHeight;
            if (this.exifdata.ImageWidth)
