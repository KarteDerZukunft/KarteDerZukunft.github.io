...
var exportMap = function (elements, map, options) {
  ...
  function saveCanvas(input, canvas, ext) {
    if (ext == "pdf") {
      ...
      var size,
        ...
        orient,
        format = "a4";
      var margin = input.getAttribute("data-margin")
        ? Number(input.getAttribute("data-margin"))
        : 0;
      // Calculate size
      ...
      var sc = Math.min(
        (size[0] - 2 * margin) / canvas.width,
        (size[1] - 2 * margin) / canvas.height
      );
      w = sc * canvas.width;
      h = sc * canvas.height;
      // Center
      ...
      input.href = canvas.toDataURL(
        "image/" + (options.format || ext),
        options.quality
      );
  }
  ...
  return elements.forEach(function (element) {
    ...
    if ("download" in element && element.download.length > 0) {
      ...
      element.addEventListener("click", function () {
        alert("Export functions are not supported by your browser...");
      });
    }
  });
};
